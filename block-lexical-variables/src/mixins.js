...
export const coreLexicalVariableScopeMixin = {
  referenceResults: function (name, prefix, env) {
    ...
    const possiblyPrefixedVarNames = thisBlock
      .getDeclaredVarFieldNamesAndPrefixes()
      .map(([varFieldName, varPrefix]) => {
    ...
    if (thisBlock.nextConnection) {
      inputResults.push(
        LexicalVariable.referenceResult(
  ...
  withLexicalVarsAndPrefix: function (child, proc) {
    ...
    if (
      child &&
      thisBlock.getInputTargetBlock(thisBlock.getScopedInputName()) === child
    ) {
      thisBlock
        .getDeclaredVarFieldNamesAndPrefixes()
  ...
  blocksInScope: function () {
    const thisBlock = this;
    const blocks = thisBlock.getInputTargetBlock(
      thisBlock.getScopedInputName(),
    );
    return (blocks && [blocks]) || [];
  },

  declaredNames: function () {
    // TODO: Not sure why we have two different names for the same thing.
  ...
  renameFree: function (freeSubstitution) {
    ...
    thisBlock.getDeclaredVarFieldNames().forEach((fieldName) => {
      ...
      if (renamedBodyFreeVars.isMember(oldVar)) {
        // Variable is bound in body.
        const newVar = FieldLexicalVariable.nameNotIn(
          oldVar,
          renamedBodyFreeVars.toList(),
        );
        const substitution = Substitution.simpleSubstitution(oldVar, newVar);
  ...
  freeVariables: function () {
    // return the free variables of this block
    const thisBlock = this;
...
export const lexicalVariableScopeMixin = {
  ...
  getDeclaredVarFieldNamesAndPrefixes: function () {
    const thisBlock = this;
    return thisBlock
      .getDeclaredVarFieldNames()
      .map((varFieldName) => [varFieldName, undefined]);
  },
};
...
export const prefixedLexicalVariableScopeMixin = {
  ...
  getDeclaredVarFieldNames: function () {
    ...
    return thisBlock
      .getDeclaredVarFieldNamesAndPrefixes()
      .map(([varFieldName, _]) => varFieldName);
  },
};

Blockly.Extensions.registerMixin(
  'lexical_variable_scope_mixin',
  lexicalVariableScopeMixin,
);
Blockly.Extensions.registerMixin(
  'prefixed_lexical_variable_scope_mixin',
  prefixedLexicalVariableScopeMixin,
);
